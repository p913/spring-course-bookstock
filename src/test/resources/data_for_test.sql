...
--Книги для тестов, иденитичные им данные содержатся в классах Etalon*****
...
--Права для добавленных книг...
--...книги как объекты доступа
insert into acl_object_identity (object_id_class, object_id_identity, parent_object, owner_sid, entries_inheriting)
  select 1, cast(book_id as varchar), null, 1, false
    from books;

--...ACL for ROLE_STOCK_KEEPER -  grant READ, WRITE, DELETE
insert into acl_entry (acl_object_identity, ace_order, sid, mask, granting, audit_success, audit_failure)
  select id, 1, 2, 1, true, true, true
    from acl_object_identity
    where object_id_identity != '0';

insert into acl_entry (acl_object_identity, ace_order, sid, mask, granting, audit_success, audit_failure)
  select id, 2, 2, 2, true, true, true
    from acl_object_identity
    where object_id_identity != '0';

insert into acl_entry (acl_object_identity, ace_order, sid, mask, granting, audit_success, audit_failure)
  select id, 3, 2, 8, true, true, true
    from acl_object_identity
    where object_id_identity != '0';

--...ACL for ROLE_READER - grant READ
insert into acl_entry (acl_object_identity, ace_order, sid, mask, granting, audit_success, audit_failure)
  select id, 5, 3, 1, true, true, true
    from acl_object_identity
    where object_id_identity != '0';
